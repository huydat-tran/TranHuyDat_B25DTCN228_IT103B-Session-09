...
const decreasePrices = prices.forEach((price, i) => {
  prices[i] = price * 0.9;
});
...
const report = names.forEach((name, i) => {
  let status;

  if (stocks[i] > 0) status = "Còn hàng";
...
Chọn chức năng:`),
  ...
  switch (choice) {
    ...
    case 5:
      findProducts();
      if (results.length === 0) alert("Không tìm thấy sản phẩm");
